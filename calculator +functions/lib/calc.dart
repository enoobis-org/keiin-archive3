  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        ElevatedButton(
          onPressed: () {
            final double firstNumber = double.tryParse(firstNumberController.text) ?? 0.0;
            final double secondNumber = double.tryParse(secondNumberController.text) ?? 0.0;
    ...
    return OperationPage(
      ...
      operationWidget: OperationInputPage(
        onOperationPerform: (double base, double exponent) {
          final double result = pow(base, exponent).toDouble(); // Fixed type mismatch
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
